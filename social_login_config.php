...
class TokenModel
{
    ...
    // private 변수를 외부에서 사용하기 위해 만든 함수
...
// 프로필 모델
// 계정의 이메일이랑 프로필 정보를 관리하는 클래스    
class ProfileModel
{
    // public은 외부에서 수정이 가능하다.
    public $nickname;
    public $email;
    public $uid;
...
function getTokenModel($code, $state)
{
    ...
    // api와 통신하는 코드 res then, catch가 비슷하다.
    try {
        // commend line 초기화
        $ch = curl_init();
        ...
        //응답받은 json 디코딩. string값을 json으로 변환한다.
...
function getProfile($accessToken, $state)
{
    ...
    // json데이터
    $response = curl_exec($ch);
    // https 통신 종료. 접속자가 많아지면 딜레이가 길어질 수 있으므로 종료를 시켜서 딜레이를 줄인다.
    curl_close($ch);
